...
@ExperimentalCoroutinesApi class SignInViewModelTest {

  @get:Rule val testRule: TestRule = InstantTaskExecutorRule()
  ...
  @Test fun `call checkSession with session then signInUiState value should GoToMainActivity`() {
  ...
  fun `check login with password equals database password then signInUiState should GoToMainActivity`() {
    val authEntity = AuthEntity(
      userName = "zaki", password = "12345", name = "Zaki Mukhammad"
    )
  ...
  @Test fun `check login with null user data then signInUiState should failed`() {
